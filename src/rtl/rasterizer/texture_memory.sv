module texture_memory #(
	...
	parameter int unsigned Bank0TextureCount = 32,
	parameter int unsigned Bank1TextureCount = 16,
	...
	localparam int unsigned TEXTURE_COUNT = Bank0TextureCount + Bank1TextureCount;
	localparam int unsigned BANK0_TEXTURE_NUMBER_WIDTH = $clog2(Bank0TextureCount);
	localparam int unsigned BANK1_TEXTURE_NUMBER_WIDTH = $clog2(Bank1TextureCount);
	localparam int unsigned BANK0_MEMORY_DEPTH = Bank0TextureCount * TextureWidth * TextureHeight;
	localparam int unsigned BANK1_MEMORY_DEPTH = Bank1TextureCount * TextureWidth * TextureHeight;
	localparam int unsigned BANK0_ADDRESS_WIDTH = $clog2(BANK0_MEMORY_DEPTH);
	localparam int unsigned BANK1_ADDRESS_WIDTH = $clog2(BANK1_MEMORY_DEPTH);
	...
	logic [TILE_WIDTH-1:0][BANK0_ADDRESS_WIDTH-1:0] bank0_read_address;
	logic [TILE_WIDTH-1:0][BANK1_ADDRESS_WIDTH-1:0] bank1_read_address;
	logic [BANK0_TEXTURE_NUMBER_WIDTH-1:0] bank0_texture_number;
	logic [BANK1_TEXTURE_NUMBER_WIDTH-1:0] bank1_texture_number;
	logic [TILE_WIDTH-1:0][7:0] bank0_texel_q;
	logic [TILE_WIDTH-1:0][7:0] bank1_texel_q;
	logic bank_select;
	logic bank_select_q;
	...
	always_comb begin
		bank_select = 1'b0;
		bank0_texture_number = '0;
		bank1_texture_number = '0;

		if (texture_id_i < TEXTURE_ID_WIDTH'(Bank0TextureCount)) begin
			bank0_texture_number = BANK0_TEXTURE_NUMBER_WIDTH'(texture_id_i);
		end else if (texture_id_i < TEXTURE_ID_WIDTH'(TEXTURE_COUNT)) begin
			bank_select = 1'b1;
			bank1_texture_number = BANK1_TEXTURE_NUMBER_WIDTH'(texture_id_i);
		end

		for (int pixel = 0; pixel < TILE_WIDTH; pixel++) begin
			bank0_read_address[pixel] = {
				bank0_texture_number,
				v_i[pixel],
				u_i[pixel]
			};

			bank1_read_address[pixel] = {
				bank1_texture_number,
				v_i[pixel],
				u_i[pixel]
			};
		end

		texel_o = bank0_texel_q;

		if (bank_select_q) begin
			texel_o = bank1_texel_q;
		end
	end
	...
	always_ff @(posedge clk_i) begin
		if (in_valid_i && in_ready_o) begin
			...
			bank_select_q <= bank_select;
		end
	end

	for (genvar memory = 0; memory < MEMORY_COUNT; memory++) begin : gen_texture_memory
		(* ram_style = "block" *) logic [7:0] bank0_texture [0:BANK0_MEMORY_DEPTH-1];
		(* ram_style = "block" *) logic [7:0] bank1_texture [0:BANK1_MEMORY_DEPTH-1];

		initial begin
			for (int texture_number = 0; texture_number < Bank0TextureCount; texture_number++) begin
				for (int v = 0; v < TextureHeight; v++) begin
					for (int u = 0; u < TextureWidth; u++) begin
						if (texture_number == 0) begin
							bank0_texture[(texture_number * TextureHeight + v) * TextureWidth + u]
			...
			for (int texture_number = 0; texture_number < Bank1TextureCount; texture_number++) begin
				for (int v = 0; v < TextureHeight; v++) begin
					for (int u = 0; u < TextureWidth; u++) begin
						bank1_texture[(texture_number * TextureHeight + v) * TextureWidth + u]
							= make_random_texel(Bank0TextureCount + texture_number, u, v);
					end
				end
			end
		end
		...
		always_ff @(posedge clk_i) begin
			if (in_valid_i && in_ready_o && !bank_select) begin
				bank0_texel_q[2 * memory]     <= bank0_texture[bank0_read_address[2 * memory]];
				bank0_texel_q[2 * memory + 1] <= bank0_texture[bank0_read_address[2 * memory + 1]];
			end

			if (in_valid_i && in_ready_o && bank_select) begin
				bank1_texel_q[2 * memory]     <= bank1_texture[bank1_read_address[2 * memory]];
				bank1_texel_q[2 * memory + 1] <= bank1_texture[bank1_read_address[2 * memory + 1]];
			end
		end
	end
