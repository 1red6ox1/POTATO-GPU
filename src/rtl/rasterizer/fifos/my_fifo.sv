...
module my_fifo #(
  ...
  // Keep the original FIFO implementation below unchanged.  These aliases
  // only translate its old write/read names to the rasterizer interface.
  logic                  clr_i;
  logic                  wvalid;
  logic                  wready;
  ...
  assign clr_i       = clear_i;
  assign wvalid      = in_valid_i;
  assign in_ready_o  = wready;
  assign wdata       = in_data_i;
  assign out_valid_o = rvalid;
  assign rready      = out_ready_i;
  assign out_data_o  = rdata;
